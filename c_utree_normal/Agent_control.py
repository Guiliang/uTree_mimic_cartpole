...
class CUTreeAgent:
    ...
    def __init__(self, problem, max_hist, check_fringe_freq, is_episodic=0):
        ...
        self.SAVE_PATH = "/Local-Scratch/UTree model/cartpole/model_control_normal_save/"
    ...
    def reward_shaping(self, pole_angle):

        if pole_angle < 0:
            return (pole_angle+0.1)/0.1
        else:
            return (0.1-pole_angle)/0.1
    ...
    def play(self):
        ...
        for game_number in range(100):
            ...
            for i in range(0, 1000):
                ...
                action = self.executePolicy(currentObs, 0.5)  # execute epsilon-greedy policy basing on Q
    ...
    def episode(self, timeout=int(1e5), save_checkpoint_flag=1):
        ...
        for game_number in range(self.max_episodes):
            ...
            for i in range(0, 1000):
                ...
                reward = self.reward_shaping(nextObs[2])
            ...
            self.utree.print_tree_structure()
            print 'game number:{0}, with epsilon:{1} and game number:{2}'.format(str(game_number), str(self.epsilon), str(i))
            if save_checkpoint_flag and game_number % 1000 == 0:
                print "*** Writing Game File {0}***".format(str(game_number + 1))
                pickle.dump(self.utree,
                            open(self.SAVE_PATH + "pickle_Game_File_" + str(game_number + 1) + ".p", 'wb'))
