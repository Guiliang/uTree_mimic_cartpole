...
class CUTreeAgent:
    ...
    def __init__(self, problem, max_hist, check_fringe_freq, is_episodic=0, training_mode=''):
        ...
        self.SAVE_PATH = '/Local-Scratch/UTree model/cartpole/model_boost_linear_qsplit_noabs_save{0}/'.format(
            training_mode)
        self.SAVE_MODEL_TREE_PATH = '/Local-Scratch/UTree model/cartpole/model_boost_add_linear_qsplit_save{0}/'.format(
            training_mode)
        self.PRINT_TREE_PATH = './print_tree_record/print_boost_linear_tree_split{0}.txt'.format(
            training_mode)
    ...
    def read_csv_game_record_list(self, csv_dir):
        dict_all = []
        with open(csv_dir, 'r') as csvfile:
    ...
    def get_Q_values(self, currentObs, nextObs, action, home_reward, away_reward, home_identifier=None):
        inst = C_UTree.Instance(-1, currentObs, action, nextObs, home_reward, away_reward)
        node = self.utree.getAbsInstanceLeaf(inst)
        return (
            node.utility(home_identifier=True), node.utility(home_identifier=False),
            node.utility(home_identifier=False, end_identifier=True))

    def get_Q_values_linear_tree(self, currentObs, nextObs, action, home_reward, away_reward, qValue,
    ...
    def boost_tree_testing_performance(self, save_path, read_game_number, save_correlation_dir, save_mse_dir,
                                       save_mae_dir, save_rae_dir, save_rse_dir):
        ...
        for game_number in game_to_print_list:
            ...
            game_record = self.read_csv_game_record(
                self.problem.games_directory + 'record_cartpole_transition_game{0}.csv'.format(int(game_number)))
    ...
    def feature_importance(self):
        ...
        self.utree.feature_influence_dict = {'Cart_Position': 0, 'Cart_Velocity': 0, 'Pole_Angle': 0,
                                             'Pole_Velocity_At_Tip': 0,
                                             'actions': 0}

        game_to_print_list = range(101, 201)
        for game_number in game_to_print_list:
    ...
    def one_shot_episode(self, save_checkpoint_flag=1):
        self.utree.hard_code_flag = True
        # if start_game > 0:
        #     self.read_Utree(game_number=start_game, save_path=self.SAVE_PATH)
        count = 0

        game_record = self.read_csv_game_record_list('../cartpole_record_dataset.csv')
        event_number = len(game_record)
        beginflag = True
        count += 1

        self.utree.test_linear_model = False

        for index in range(1, int(0.8 * event_number)):

            if self.problem.isEpisodic:
                currentObs = game_record[index - 1][:-2]
                nextObs = game_record[index][:-2]
                reward = float(1)
                action = float(game_record[index - 1][-2])
                qValue = float(game_record[index - 1][-1])

                if index == event_number - 1:  # game ending
                    self.utree.test_linear_model = True
                    print >> sys.stderr, '=============== final update starts ==============='
                    # tracker.print_diff()
                    self.update(currentObs, nextObs, action, reward, qValue, value_iter=1, check_fringe=1,
                                beginflag=beginflag)
                    # tracker.print_diff()
                    print >> sys.stderr, '=============== final update finished ===============\n'

                elif index % 400 == 0:  # game ending
                    self.utree.test_linear_model = True
                    print >> sys.stderr, '=============== update starts ==============='
                    # tracker.print_diff()
                    self.update(currentObs, nextObs, action, reward, qValue, value_iter=1, check_fringe=1,
        ...
        if self.problem.isEpisodic:
            ...
            if save_checkpoint_flag:
                pickle.dump(self.utree,
                            open(self.SAVE_PATH + 'one_shot_pickle_Game_File.p', 'wb'))
