    ...
    with open(csv_dir, 'rb') as file:
        ...
        for row in rows:
            if row == '\n':
                ...
                read_counter += 1
                if read_counter % 3 == 1:
                    linear_list.append(float(correlations[0].strip().split(':')[1]))
                elif read_counter % 3 == 2:
                    oracle_list.append(float(correlations[0].strip().split(':')[1]))
                elif read_counter % 3 == 0:
                    merge_list.append(float(correlations[0].strip().split(':')[1]))
    return linear_list, oracle_list, merge_list
...
def record_transition_length():
    ...
    for i in game_number:
        game_file_dir = '../save_all_transition/record_cartpole_transition_game{0}.csv'.format(str(i))
...
def plot_mae_shadow_graph():
    csv_dir_list = ['./result/result-mae-all-linear-epoch-decay-lr-st0-300',
                    './result/result-mae-all-linear-epoch-decay-lr-st0-400',
                    './result/result-mae-all-linear-epoch-decay-lr-st0-500',
                    './result/result-mae-all-linear-epoch-decay-lr-st0-600',
    ...
    for csv_dir in csv_dir_list:
        home_linear_mae, home_oracle_mae, home_merge_mae = read_result_csv(
            csv_dir=csv_dir)
        x_plot_list, y_plot_list = dealing_plot_data(home_linear_mae, transition_length_list)
...
def plot_rmse_shadow_graph():
    csv_dir_list = ['./result/result-mse-all-linear-epoch-decay-lr-st0-300',
                    './result/result-mse-all-linear-epoch-decay-lr-st0-400',
                    './result/result-mse-all-linear-epoch-decay-lr-st0-500',
                    './result/result-mse-all-linear-epoch-decay-lr-st0-600',
                    './result/result-mse-all-linear-epoch-decay-lr-st0-700'
                    ]
    x_plot_array = None
    y_plot_array = None
    transition_length_list = record_transition_length()
    for csv_dir in csv_dir_list:
        home_linear_rmse, home_oracle_rmse, home_merge_rmse = read_result_csv(
            csv_dir=csv_dir)
        x_plot_list, y_plot_list = dealing_plot_data(home_linear_rmse, transition_length_list)

        y_plot_rmse_list = []
        for y in y_plot_list:
            y_plot_rmse_list.append(y**0.5)

        x_plot_array = np.asarray(x_plot_list)
        if y_plot_array is None:
            y_plot_array = np.asarray([y_plot_rmse_list])
        else:
            y_plot_array = np.concatenate((y_plot_array, np.asarray([y_plot_rmse_list])), axis=0)

    plot_tree_shadow_result(x_plot_array, y_plot_array, "RMSE")


def plot_correl_shadow_graph():
    csv_dir_list = ['./result/result-correlation-all-linear-epoch-decay-lr-st0-300',
                    './result/result-correlation-all-linear-epoch-decay-lr-st0-400',
                    './result/result-correlation-all-linear-epoch-decay-lr-st0-500',
                    './result/result-correlation-all-linear-epoch-decay-lr-st0-600',
...
if __name__ == "__main__":
    plot_rmse_shadow_graph()
    # plot_mae_shadow_graph()
